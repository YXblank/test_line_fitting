...
public:
  ...
  { return Point(r * cos(phi), r * sin(phi)); }    //转换为坐标（x,y）

  double length()        const { return sqrt(pow(x, 2.0) + pow(y, 2.0)); }        //求距离
  double lengthSquared() const { return pow(x, 2.0) + pow(y, 2.0); }      //x*x+y*y
  double angle()         const { return atan2(y, x); }
  double angleDeg()      const { return 180.0f * atan2(y, x) / pi; }
  double dot(const Point& p)   const { return x * p.x + y * p.y; }   //点乘
  double cross(const Point& p) const { return x * p.y - y * p.x; }   //叉乘
